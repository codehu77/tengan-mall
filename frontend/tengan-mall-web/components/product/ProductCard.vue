<template>
  <NuxtLink :to="`/item/${product.spuId ?? product.skuId}`" class="block h-full">
    ...
    >
      <!-- 商品圖片：固定像素高度，跟上傳圖片寬高無關；用 object-contain 完整顯示置中，不裁切
           (曾用 aspect-square 讓框跟著卡寬變正方形，但 Grid 計算列高時會被圖片原始尺寸干擾，框變不成正方形) -->
      <div class="relative h-[200px] shrink-0 bg-card overflow-hidden">
        <img
      ...
      <div class="flex flex-col flex-1 p-3">
        <p ref="nameEl" class="flex items-end text-sm text-body leading-[1.4] h-10 mb-1.5 overflow-hidden break-all" :title="product.skuName">
          {{ displayName }}
        </p>
