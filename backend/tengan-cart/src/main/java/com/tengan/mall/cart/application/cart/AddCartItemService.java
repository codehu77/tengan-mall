 ...
 * 不變條件，見 cart_storage_decision。開賣時間閘門在這裡擋（限購不在加入購物車時強制擋，
 * 真正的限購以下單時 tengan-inventory 的原子檢查為準）。
 */
@Service
public class AddCartItemService implements AddCartItemUseCase {
    ...
    public AddCartItemResult add(AddCartItemCommand command) {
        if (!inventoryAvailabilityPort.isPurchasable(command.skuId())) {
            throw new SkuNotYetOnSaleException(command.skuId());
        }
