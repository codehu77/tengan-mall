 ...
 * 列的全部 scope 一次註冊完。使用者要求 devtools 匯入時順便幫新 SKU 建庫存，`tengan-devtools` 再多
 * inventory.read（列倉庫，找預設倉）+ inventory.write（建庫存列）。Phase 8 新增 tengan-wallet 網域：
 * `tengan-wallet` 本身不主動呼叫任何服務
