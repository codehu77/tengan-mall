...
/** 換發/快取呼叫 tengan-cart 用的 Service JWT（比照 tengan-cart 呼叫 tengan-product 的既有模式）。 */
@Component
public class CartServiceTokenProvider {

    private static final String CLIENT_REGISTRATION_ID = "tengan-cart";

    private final OAuth2AuthorizedClientManager authorizedClientManager;

    public CartServiceTokenProvider(OAuth2AuthorizedClientManager authorizedClientManager) {
    ...
    public String getAccessToken() {
        ...
        if (authorizedClient == null) {
            throw new IllegalStateException("無法取得呼叫 tengan-cart 用的 Service JWT");
        }
