...
/** 換發/快取呼叫 tengan-search 用的 Service JWT，跟 ProductServiceTokenProvider 同一個模式、不同 client。 */
@Component
public class SearchServiceTokenProvider {

    private static final String CLIENT_REGISTRATION_ID = "tengan-search";

    private final OAuth2AuthorizedClientManager authorizedClientManager;

    public SearchServiceTokenProvider(OAuth2AuthorizedClientManager authorizedClientManager) {
    ...
    public String getAccessToken() {
        ...
        if (authorizedClient == null) {
            throw new IllegalStateException("無法取得呼叫 tengan-search 用的 Service JWT");
        }
